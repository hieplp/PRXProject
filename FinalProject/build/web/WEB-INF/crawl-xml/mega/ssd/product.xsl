...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"
    ...
    <xsl:template match="t:ssds">
        <xsl:variable name="listDoc" select="document(@link)"/>
        <xsl:element name="ssds">
    ...
    <xsl:template name="itemDetailList">
        ...
        <xsl:for-each select="$items">
            ...
            <xsl:if test="(number($price) = $price) and (string-length($capacity) > 0) and (string-length($interface) > 0)">
                
                <xsl:element name="ssd">
